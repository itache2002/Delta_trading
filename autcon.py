...
class Delat():
    def __init__(self,api_key, api_secret,lev):
        ...
        self.finaldf=None
        self.livedf= None
         # Flag to track whether the program should continue running
        self.running = True
    ...
    def send_heartbeat(self, ws):
        while True:
            ...
            if ws.sock and ws.sock.connected:
                ...
                # Wait for a certain interval before sending the next heartbeat
    ...
    def Strategy(self,close_data):
        ...
        if len(self.livedf) >= 2 and len(self.historydf) >= 2:
            if not pd.isna(self.livedf['current_EMA'].iloc[-2]) and not pd.isna(self.livedf['current_SMA'].iloc[-2]):
                ...
                if (self.livedf['current_EMA'].iloc[-2] < self.livedf['current_SMA'].iloc[-2]) and (diff <= 5):
                    print("##################")
                    print(" BUY Signal ")
                    print("##################")
                    self.entry = float(self.livedf['close'].iloc[-1])
                    self.stoploss= int(self.entry  - 200)
                    ...
                    print(f"The takeprofit is set at {self.exit}")
    ...
    # Send authentication message
        ...
        heartbeat_thread = threading.Thread(target=self.send_heartbeat, args=(ws,))
        ...
        ''' 
        if u want to change the candle_stick time u can  change candlestick_1m for example:
    ...
    def live_data(self):
        signal.signal(signal.SIGINT, self.on_signal)  # Handle Ctrl+C signal
        while True:
            ...
            time.sleep(10)  # Wait before attempting reconnection
        ws.close()
